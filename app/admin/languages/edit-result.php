...
# update
if(!$Admin->object_modify("lang", $POST->action, "l_id", $values))	{ $Result->show("danger",  tr_("Language %s error", _($User->get_post_action()) ), true); }
else																	{ $Result->show("success", tr_("Language %s success", _($User->get_post_action())), true); }
