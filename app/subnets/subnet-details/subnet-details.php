...
<table class="ipaddress_subnet table-condensed table-full">
	...
	<tr>
		...
		if (isset($vlan['domainId'])) {
    		...
    		if($l2domain!==false)       { print " <span class='badge badge1 badge5' rel='tooltip' title='"._('VLAN is in domain')." "._($l2domain->name)."'>"._($l2domain->name)." "._('Domain')." </span>"; }
        }
		?>
		</td>
	</tr>
	...
	<tr>
		...
		if(!empty($subnet['customer_id'])) {
			...
			if ($customer!==false) {
				print $customer->title." <a target='_blank' href='".create_link("tools","customers",unescape_input($customer->title))."'><i class='fa fa-external-link'></i></a>";
			}
			else {
				print "<span class='text-muted'>/</span>";
			}
